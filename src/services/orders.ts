...
import { getYocoCheckout, isYocoPaidStatus } from "@/lib/yoco";
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (firstInstallment) {
      await tx.paymentInstallment.update({
        ...
        data: { status: "PAID", paidAt: new Date(), paymentReferenceId: paymentReference }
      });
    }
    ...
    const order = await tx.order.update({
      ...
      data: {
        ...
        paidAt: new Date(),
        yocoPaymentId: paymentReference
      },
      include: { items: true, tickets: true }
    });
...
export async function syncPaidOrderFromYoco(orderId: string, paymentReference?: string) {
  const order = await prisma.order.findUnique({
    where: { id: orderId },
    select: { id: true, status: true, yocoCheckoutId: true }
  });

  if (!order?.yocoCheckoutId || order.status !== "PENDING") return;

  await syncPaidOrderFromYocoCheckout(order.yocoCheckoutId, paymentReference);
}

export async function syncPaidOrderFromYocoCheckout(checkoutId: string, paymentReference?: string) {
  const order = await prisma.order.findUnique({
  ...
  if (!isYocoPaidStatus(checkout.status)) return;

  await markOrderPaid(order.id, paymentReference ?? checkout.id);
}
